...
import type { VehicleStatus } from "@lacity/shared";
 ...
 * the durable twice-daily sweep. Missing freight remains parked indefinitely.
...
export function createFreightProcessor(deps: FreightDeps) {
  ...
  return async (job: Job<FreightJobData>): Promise<void> => {
    ...
    await publishVehicle(publisher, updated);
    logger.info({ vehicleId, attempts, reason: result.reason }, "Freight miss; parked for twice-daily sweep");
  };
}
