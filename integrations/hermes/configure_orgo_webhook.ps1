param(
    ...
    [string]$LatestChromeDownloadHelper = "$PSScriptRoot\latest_chrome_download.py",
...
foreach ($required in $hermes, $config, $envFile, $RagRoot, $PromptFile, $CallbackHelper, $RdpFocusHelper, $AutoSoftPinHelper, $BatchSourceHelper, $BatchManifestHelper, $BatchPostingManifestHelper, $BatchDecodeHelper, $BatchCheckpointHelper, $LatestChromeDownloadHelper, $GatewayWatchdogPatch, $python) {
...
Copy-Item -LiteralPath $LatestChromeDownloadHelper -Destination (Join-Path $toolsDir 'latest_chrome_download.py') -Force
...
- Use literal `STOCK SHEET LA CITY CARS` as the `latest_chrome_download.py` filename prefix for LA City and literal `STOCK SHEET COLUMBIA` for Columbia City. Never derive that prefix from a decorated browser/tab title, including a title with an emoji or ` - Google Sheets`.
...
- Preflight every supplied vehicle first. Accept complete dashboard freight evidence without reopening dispatch. Export and scan only the payload store's stock sheet; never fetch the other store's sheet during a store-isolated run. A fresh current-store stock-sheet export is mandatory before the first manifest preflight in every webhook run. Record a fresh UTC marker immediately before initiating the authenticated Google Sheets XLSX export. Browser Use can save that file under an older named-session workspace rather than `Downloads`; resolve it only with `python tools/latest_chrome_download.py --filename-prefix <exact current-store workbook name> --since-utc <current-run export marker> --output <sheet-download.json>`, require `ready: true`, and use only its returned absolute `path` as `<target-store-export>`. Never enumerate `Downloads`, choose a workbook by filename/mtime, reuse a prior run's path, or infer the path from Chrome's downloads page. If the helper finds no completed matching download, retry the authenticated export once with a new marker and resolve once more, then fail closed. Apply the same marker-and-resolver procedure to the independent post-write export/readback; the readback workbook must have a download start after the write completed. Before opening NextGear, run `tools/batch_source_preflight.py` against the newest Exportable Inventory workbook and all payload VINs. Reuse it when the helper returns `ready: true` (at most six hours old, one exact active approved row per VIN). `In Stock`, `At Auction`, and `Collateral Verified` are active vehicle statuses; when Floorplan Status is present it must be `Approved`. Otherwise open NextGear directly at Tools > Exportable Inventory and make one fresh export; never substitute Approved Floorplans for ACV/source evidence. Run the helper again and fail closed if it is still not ready. Then run `tools/batch_manifest_preflight.py --request <canonical-current-payload.json> --stock-sheet <resolved-current-store-export> --stock-prefix <payload store.stock_prefix> --expected-request-id <current webhook request_id> --output <manifest-validation.json>` and require `ready: true`; never create or debug a validator during a live run.
- Resolve color only for candidates whose configured sheet color cell is absent or blank. A `REUSE_EXISTING` row with a nonblank value in the store-configured color column already supplies verified color and must not be searched again. For only the remaining VINs, when the reusable export lacks color, use the authenticated NextGear Floor Plans list as a supplement. Use Browser Use only with the fixed named session `vehicle-stocking` so webhook runs reuse the approved existing-profile connection; never create an unnamed or alternate Browser Use session. On this 1-vCPU host, every `browser_exec` call MUST use `timeout_s` of at least 300 seconds and one call MUST combine tab discovery, navigation, every VIN search, and compact result output for the whole source-site transaction; never spend separate calls merely inspecting, clicking, or printing page state. At the start of that call inspect the existing tabs and reuse the tab whose URL/title proves the authenticated NextGear dealer application. Never call `new_tab` for the public NextGear homepage while an authenticated dealer tab exists. If that named connection is unavailable, use native saved-login control. If Cox shows the saved-login flow, verify the expected saved username is already populated, click Next once, require a masked/autofilled password, and click Sign in once. Never type, copy, reveal, or persist the password; fail closed if it is not autofilled or authentication is rejected. Open Floor Plans once and search each still-missing full VIN sequentially in the same semantic search field. Require exactly one result and record the color shown in its Description; clear/replace the search value for the next VIN. If a fresh verification capture proves the field still contains the prior VIN (or is blank) after the first entry, perform exactly one recovery: refocus the verified field, Ctrl+A, paste the exact current VIN once, submit once, and capture the result. This is a verified replacement, not a blind duplicate; never make a third entry attempt. Do not open detail pages and do not rerun exports when the one-result list supplies color.
- Keep the entire Browser Use `code` argument below 3,500 characters so the gateway cannot truncate it into invalid Python. Emit exactly one compact JSON line with `print(json.dumps(result, separators=(",", ":")))`; never place a literal multiline string inside a single-quoted Python value. If the transaction cannot fit under that limit, skip Browser Use and go directly to native saved-profile control instead of retrying truncated code.
...
- In the legacy purchase entry, currency fields accept ordinary dollar values with an optional decimal (`500.00` is typed as `500.00`, never `50000`; `20620.00` is typed as `20620.00`, never `2062000`). After each value loses focus, require the displayed amount to equal the intended dollars and cents. Enter the debit template and freight first, set Invoice Amount to the vehicle ACV, then enter the credit template. Leave the inventory debit row amount alone: AutoSoft dynamically balances it back to full ACV as credits are committed. Freight belongs on the credit row whose GL is 31105; never put 31105 on the reserved Cash Purchase row, and verify every nonzero amount has the intended GL before Post. Decode before touching Line. If expected `9T` and GL 24100 are already visible, do not re-edit Line. If Line must be set, treat its one-character numeric and suffix controls separately with one direct key each (no Ctrl+A or clipboard paste in those two controls), commit with Tab, and re-verify `9T`, Used, and GL 24100 before posting. On blank and saved-record readback screens, the long selector immediately to the right of the two authoritative Line character boxes can keep showing an unrelated default such as `9C 24300 USED CAR`; never interpret that selector label as the saved Line or GL. Read Line only from the two character boxes and GL only from the accounting row GL field. A saved readback may clear the posted debit/credit grid; verify stock, VIN, the Line character boxes, Invoice Amount/ACV, and the separately saved Vehicle Information instead of treating that blank grid as a failed post. A clean blank Vehicle Purchases form immediately after Save is the expected successful reset, not a disappeared window or failure; reopen the saved record by current stock in read-only lookup and verify it without re-entering or reposting. AutoSoft Source accepts at most 20 characters: use only manifest `source_autosoft`, clear it once with Ctrl+A then Backspace, verify blank, paste once, and verify the exact retained value before Save. Never append to a retained Source value or make repeated blind attempts.
- `Post Direct` is asynchronous. Click `Post Transaction` once and select the current accounting period exactly once, then wait through the visible `Post Direct / Posting Auto Inventory` job until it either reaches Vehicle Information or returns control. Never click the period twice, click Post again, or clear the purchase form while that job is running. If `Stock No. ... Is In Use And I'm Trying To Post` or `Clear That Screen So I Can Continue` appears while the same Post Direct job is visibly active, acknowledge each modal at most once and keep waiting; do not classify the vehicle from the modal alone. After the job returns, reopen the current stock in Vehicle Purchases and Inventory > Vehicle Internals before deciding the outcome. Treat it as successfully posted when the exact stock/VIN has the expected Original/Total Inventory, expected Floor Plan ACV, and the expected dated debit lines. If miles, color, or source are blank after that verified post, use the normal Inventory > Info record to enter only those manifest values, leave with Back so AutoSoft persists them, reopen Info, and verify them; this is completion of the existing record, never a repost or Edits & Corrections. Only apply the zero-shell recovery rule when fresh Vehicle Internals proves zero original inventory, zero internals, and no dated journal lines.
...
- Use `tools/batch_checkpoint.py record` for every terminal manifest/checkpoint update, commit and push those exact files with terminal prompting disabled, finish all RAG evaluation and cleanup, then use `tools/batch_checkpoint.py callback` with `--callback-output` outside the RAG Git tree under `C:\ProgramData\VehicleStocking\callbacks` and `tools/dashboard_callback.py --payload-file ...`. Use a callback filename made from the safe batch ID, nonce, and position. Files whose names contain the current parent request ID are the current run's own artifacts, not concurrency. Never hand-author, hand-patch, or repair JSON during a live run. Once the final terminal callback is accepted/idempotent, call no more tools and immediately return the concise batch result.
