...
- Before a contiguous live-input phase inside Remote Desktop, run `python tools/focus_autosoft_rdp.py --expected-title <payload store.autosoft_instance>`, require `ok: true`, capture the fresh screen, and visually verify the intended control. Keep that HWND foreground; re-run the guard at each new vehicle, after any window/focus change, or when a fresh capture cannot prove state. Never send background input to `mstsc.exe`. An `unverifiable` action requires a fresh visual capture before more input, not repeated blind clicking.
...
- Acquire a foreground RDP lease with `python tools/focus_autosoft_rdp.py --expected-title "<payload store.autosoft_instance>"` before AutoSoft and at the start of each vehicle. Continue only when it returns `ok: true`, capture the fresh screen, and visually prove the intended control. Keep that verified HWND foreground across consecutive actions; re-run after a focus/window change or unexplained screen. Never send background input to `mstsc.exe`, and never type after an `unverifiable` action until a fresh capture proves state.
